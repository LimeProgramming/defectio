...
*defectio* logs errors and debug information via the :mod:`logging` python
...
example to write the logs to a file called ``defectio.log`` instead of
outputting them to the console the following snippet can be used::
    ...
    handler = logging.FileHandler(filename='defectio.log', encoding='utf-8', mode='w')
